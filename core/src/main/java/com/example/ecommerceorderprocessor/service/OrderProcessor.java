...
public class OrderProcessor {
    ...
    public void process(final Order order) {
        log.info("Starting to process order {} with status {}", order.getOrderId(), order.getStatus());

        try {
            // send to CRM System (all order statuses)
            log.info("Sending order {} to CRM", order.getOrderId());
            crmService.sendOrderUpdate(order);
            log.info("Successfully sent order {} to CRM", order.getOrderId());
            ...
            if (order.getStatus() == OrderStatusEnum.PAID || order.getStatus() == OrderStatusEnum.CANCELLED) {
                log.info("Sending order {} to Financial system", order.getOrderId());
                financialService.writeOrderToFile(order);
                log.info("Successfully sent order {} to Financial system", order.getOrderId());
            }
        } catch (Exception e) {
